...
- **Validation Command**: Run in terminal `task check` this command will trigger Typechecker `tsgo --noEmit` and lint `biome check --write` with custom rules to enforce no `any` usage and other GZANSP constraints
