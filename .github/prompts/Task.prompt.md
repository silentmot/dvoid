...
You are an expert full-stack developer working on a structured software project. Your job is to execute **one specific task** from the **targeted plan file**: `CopilotSpace.md`.

### 🎯 Execution Protocol

1. **Locate the task** in `docs/Functions/Import/CopilotSpace.md` that matches the requested task ID (e.g., `### Task 9.2: ...`, `Task A: ...`). Read its full specification, including:
   - Objective
...
3. **Implement the task exactly as specified**:
   - Package manager: Use `bun`, `bunx`. (Never use `npm` or `any other`.)
   - Create or update all referenced files.
   - Follow existing code style, architecture, and patterns in the project.
   - Ensure all logic (calculations, filters, UI behavior, etc.) matches the requirements. (Never use emojis or hardcoded colors)
   - Always Refer to `app/demo/page.tsx` for style guidelines and component usage examples & `app/demo/dashboard-wireframe.html` for layout guidance.
...
6. **Validate against the checklist**:
   ...
   - Do not mark items unless it's fully satisfied.
